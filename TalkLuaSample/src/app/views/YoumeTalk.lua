...
cc.exports.youmetalk.OnEvent = function( event, errorcode, channel, param )
	...
	elseif event == 2 then --YOUME_EVENT_JOIN_OK:
        print("进入房间成功")
		addTips( "进入房间成功, channelID: "..channel.." UserID:"..param)
	...
	elseif event == 20 then 
		print("其他用户进入讲话状态，userid："..param)
	elseif event == 21 then 
		print("其他用户结束讲话，userid："..param)
	elseif event == 22 then 
		print("自己麦克风的录音音量级别，数值："..errorcode)
	elseif event == 23 then 
		print("麦克风被其他用户打开，userid："..param)
	elseif event == 24 then 
		print("麦克风被其他用户关闭，userid："..param)
	elseif event == 25 then 
		print("扬声器被其他用户打开，userid："..param)
	elseif event == 26 then 
		print("扬声器被其他用户关闭，userid："..param)
	elseif event == 27 then 
		print("取消屏蔽某人语音，userid"..param)
	elseif event == 28 then 
		print("屏蔽某人语音，userid"..param)
	end
end

-- RestApi回调
cc.exports.youmetalk.OnRequestRestApi  = function( requestid, errcode, command, result )
